...
namespace Purp3D
{
	Renderer::Texture Renderer::CreateTexture(int width, int height)
	{
		Texture result;
		result.Width = width;
	...
	Renderer::Texture Renderer::LoadTexture(const std::filesystem::path& path)
	{
	...
	Renderer::Framebuffer Renderer::CreateFramebufferWithTexture(const Texture texture)
	{
		Framebuffer result;
	...
	bool Renderer::AttachTextureToFramebuffer(Framebuffer& framebuffer, const Texture texture)
	{
	...
	void Renderer::BlitFramebufferToSwapchain(const Framebuffer framebuffer)
	{
